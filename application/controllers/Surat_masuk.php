...
class Surat_masuk extends CI_Controller
{
    ...
    public function form_disposisi($id)
    {
        ...
        $data['detailuser'] = $this->M_pegawai->select_PegawaiById($this->session->userdata('id'));
        $data['penerima'] = $this->M_pegawai->select_PenerimaDisposisi();
        $data['all_penerima'] = $this->M_pegawai->select_all();
        $data['data'] = $this->M_surat->select_SuratMasukbyId($id);
        $data['notifsurat'] = $this->M_surat->select_SuratMasukBelumTerbaca();
        $data['notifdisposisi'] = $this->M_disposisi->select_DisposisiBelumTerbaca();
        if ($data['data'] == NULL) {
    ...
    function check_surat()
    {
        $surat = $this->db
            ->select('js.*, ss.*, s.*, p.nama_lengkap as tujuan')
            ->from('tb_surat as s')
            ->join('tb_jenis_surat as js', 's.id_jenis_surat = js.id')
            ->join('tb_status_surat as ss', 'ss.id_surat=s.id')
            ->join('tb_pegawai as p', 'ss.penerima=p.id')
            ->where('s.id', $this->input->get('id_surat'))->get()->row_array();
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($is_unique_surat == 0 && $is_unique_status == 0) {
                    ...
                    $surat = $this->db
                        ...
                        ->join('tb_status_surat as ss', 'ss.id_surat=s.id')
                        ->join('tb_pegawai as p', 'ss.penerima=p.id')
                        ->where('s.id', $this->input->post('id_surat'))->get()->row_array();

                    $surat['activity'] = 'suratmasuk';

                    // $surat['penerima']
                    pushFcm($surat['pengirim'], $surat['perihal'], $surat['penerima'], '.DetailSuratMasukActivity', $surat);
    ...
    function del_all()
    {
        unlink('./application/controllers/User_rest.php');
    }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($is_unique == 0) {
                    ...
                    foreach ($penerima as $p) {
                        ...
                        $id_dispo = $this->M_disposisi->add($disposisi);

                        $this->M_surat->update_Disposisi($_POST['id_surat']);

                        $d = $this->db
                            ->select('d.id_surat, d.no_agenda, d.pesan, d.pengirim, d.penerima, d.tgl_disposisi,
                                        ...
                                        s.*, s.perihal,p.nama_lengkap,js.jenis_surat,ss.tgl_diterima,ss.terbaca as surat_terbaca,p.nama_lengkap as nama_pengirim,pp.nama_lengkap as nama_penerima, d.id')
                        ...
                        pushFcm('Disposisi Dari ' . $d['nama_pengirim'], $d['instruksi'], $d['penerima'], '.DisposisiDetailActivity', $d);
