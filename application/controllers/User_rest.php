...
class User_rest extends CI_Controller
{
    ...
    function profil_pegawai(){
        ...
        $data = $this->db
                ...
                telp, jenis_kelamin, golongan, agama, tempat_lahir, tgl_lahir, j.jabatan as nama_jabatan')
    ...
    function tindaklanjut_file(){
        $foto = $this->upload_foto();
        $data = array(
            'id_surat' => $_GET['id_surat'],
            'statusjawaban' => $_GET['statusjawaban'],
            'jawaban' => $_GET['jawaban'],
            'lampiranjawaban' => str_replace(" ", "", $_GET['lampiranjawaban'])
        );

        $this->db->where('id_surat',$data['id_surat'])->update('tb_disposisi',$data);
    ...
    function upload_foto(){
        ...
        $config['file_name']             = str_replace(" ", "", $_GET['lampiranjawaban']);
        //$config['max_width']            = 1024;
        //$config['max_height']           = 768;
        $this->load->library('upload', $config);
        // $this->upload->do_upload('filename');
        if (!$this->upload->do_upload('filename')) {
            $resp = array('status' => false, 'message' => $this->upload->display_errors());
        }else{
            $resp = array('status' => true, 'message' => 'Berhasil Ditindaklanjuti');
        }
        return $resp;
    }
    ...
    public function penerima_disposisi_search(){
        $lvl = $this->input->get('level');
        $unit = $this->input->get('unit');
        $keyword = $this->input->get('keyword');
        $id_pegawai = $this->input->get('id_pegawai');
        $jabatan = $this->input->get('jabatan');
        $result = $this->M_api->select_PenerimaDisposisi_API($lvl,$unit,$jabatan,$id_pegawai,$keyword);
        echo json_encode($result);
    }
    ...
    public function penerima_disposisi(){
        ...
        $id_pegawai = $this->input->get('id_pegawai');
        $jabatan = $this->input->get('jabatan');
        $result = $this->M_api->select_PenerimaDisposisi_API_search($lvl,$unit,$jabatan,$id_pegawai);
        echo json_encode($result);
    }
